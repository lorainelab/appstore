...
def process_jar(jar_file):
    ...
    try:
        ...
        raise ValueError('There was a problem with the file you entered - %s. It does not appear to be a proper "jar" '
                         '(java archive) file. Try downloading and uncompressing it on your local computer to check it.'
                         % jar_file)
...
def _get_manifest_file(zip_archive):
    try:
        ...
        raise ValueError('The file you submitted %s lacks a MANIFEST.MF file' % _MANIFEST_FILE_NAME)

    if manifest_info.file_size > _MAX_MANIFEST_FILE_SIZE_B:
        raise ValueError('The file you submitted has a manifest file that\'s too large; it can be at most %d bytes but '
                         'is %d bytes' % (_MAX_MANIFEST_FILE_SIZE_B, manifest_info.file_size))
    ...
    try:
        ...
        raise ValueError('The file you submitted does not have an accessible manifest file located in <tt>%s</tt>' % _MANIFEST_FILE_NAME)
...
def _get_repository_file(zip_archive):
    ...
    try:
        ...
        raise ValueError('The file you submitted <tt>%s</tt> lacks repository.xml OBR index file.' % _REPOSITORY_FILE_NAME)
    ...
    try:
        ...
        raise ValueError('The file you submitted <tt>%s</tt> contains inacessible repository.xml OBR index file.' % _REPOSITORY_FILE_NAME)
...
def _get_name_and_version(manifest, name_attr, version_attr):
    ...
    if not app_version:
        raise ValueError('The file you submitted does not have <tt>%s</tt> in its manifest' % version_attr)
    if not VersionRE.match(app_version):
        raise ValueError('The file you submitted <tt>%s</tt> does not follow this format: <i>major</i>'
                         '[.<i>minor</i>][.<i>patch</i>][.<i>tag</i>]' % version_attr)
