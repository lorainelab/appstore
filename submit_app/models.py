...
class AppPending(models.Model):
    ...
    def make_release(self, app):
        ...
        previous_release = copy.copy(Release.objects.filter(app=app).extra(select={'natural_version': "CAST(REPLACE(Bundle_Version, '.', '') as INTEGER)"}).order_by('-natural_version')[:1])
